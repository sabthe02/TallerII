	...
	public ArrayList<VOListadoBoletos> listadoBoleto(boolean esEsp) {
		...
		for (int i = 0; i < this.tope; i++) {
			...
			if ((bol instanceof BoletoEsp) && (esEsp == true)) {
				VO = new VOListadoBoletos(bol.getNombrePasajero(), bol.getEdad(), bol.getNumeroCel(), true,
						((BoletoEsp) bol).getDescuentoEsp(), bol.getNumeroBoleto());
				VOListadoBoletosL.add(VO);
			} else {
				if (esEsp == false && !(bol instanceof BoletoEsp)) {
					VO = new VOListadoBoletos(bol.getNombrePasajero(), bol.getEdad(), bol.getNumeroCel(), false, 0.0,
							bol.getNumeroBoleto());
					VOListadoBoletosL.add(VO);
				}
			}
